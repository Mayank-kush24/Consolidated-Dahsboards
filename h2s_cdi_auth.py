...
def _origin_only(url: str) -> str:
    """Return ``scheme://host[:port]`` with no path (avoids ``/dashboard`` hitting Apache/XAMPP)."""
    s = (url or "").strip()
    if not s:
        return s
    parts = urlsplit(s)
...
def get_portal_dashboard_url() -> str:
    """
    Portal home URL for “Back to CDI dashboard” and similar exits.

    Uses only ``H2S_CDI_URL`` / ``JARVIS_URL`` (not ``H2S_CDI_PUBLIC_URL`` or proxy
    headers), normalized to origin, so e.g. ``http://h2s.tech`` is not replaced by
    ``https://h2s.tech/dashboard/`` (which may be XAMPP, not the CDI app).
    """
    return _origin_only(_PORTAL_URL)
...
def _routing_path() -> str:
    ...
    prefix (e.g. /my-module/api/...), strip SCRIPT_NAME so public_paths like
    /api/internal/live-events still match.
    """
...
def get_module_event_allowlist(user: dict | None = None) -> list[str] | None:
    ...
    []    → no events.
    [...] → explicit allow-list (names must match the sheet).
    """
    if user is None:
        user = get_user()
    if user is None:
